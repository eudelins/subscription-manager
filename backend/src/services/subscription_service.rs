...
use sqlx::{Acquire, Transaction, Postgres};
...
pub async fn create_or_update_subscription(
    ...
    if let Some(res_dto_inner) = result_dto {
        if is_updated {
            let successful_remove = subscription_repository::remove_subscription_from_all_categories(
                &mut transaction,
                res_dto_inner.id
            ).await.is_some();
...
async fn rollback_and_return_none<T>(transaction: Transaction<'_, Postgres>) -> Option<T> {
    let _ = transaction.rollback().await.map_err(|err| {
        println!("Rollback error : {:?}", err)
    });
    Option::None
} 