use crate::{SubscriptionsDb, services::category_service, dto_entities::category_dto::CategoryDTO};
...
pub async fn find_category_by_id(
    ...
    category_service::find_category_by_id(db, id).await.map(Json)
}
...
pub async fn create_category (
    ...
    category_service::create_category(db, new_category.into_inner()).await.map(Json)
}
...
mod test {
    ...
    fn find_all_categories_test() {
        ...
        let response = client.get(uri!("/categories", super::find_all_categories)).dispatch();
    ...
    fn create_category_find_by_id_and_delete_by_id_test() {
        ...
        let response = client.post(uri!("/categories", super::create_category))
        ...
        let response = client.get(format!("/categories/{}", category.id))
            .dispatch();
        assert_eq!(response.status(), Status::Ok);
        let category = response.into_json::<CategoryDTO>().unwrap();
        assert_eq!(category.name, "test_new_category");
        ...
        let response = client.delete(format!("/categories/{}", category.id))
            .dispatch();
        assert_eq!(response.status(), Status::Ok);
    }
}